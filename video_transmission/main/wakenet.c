...
static void record_10s_task(void *arg)
{
    ...
    size_t offset = 0;
    const int chunk = BYTES_PER_SECOND / 2;  // ~100 ms per read
...
esp_err_t wakenet_start(void)
{
	...
	audio_recorder_wakenet_enable(s_recorder, true);
    audio_recorder_vad_check_enable(s_recorder, true);
    audio_recorder_trigger_start(s_recorder);
